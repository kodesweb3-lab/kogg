...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Simple auth check - allow in development, require token in production
  if (process.env.NODE_ENV === 'production') {
    const authHeader = req.headers.authorization;
    if (authHeader !== `Bearer ${process.env.SEED_AUTH_TOKEN}`) {
  ...
  try {
    ...
    for (const entry of devLogs) {
      // Check if entry already exists (by title and publishedAt)
      ...
      if (!existing) {
        ...
        created.push(devLog);
        logger.info(`Created dev log: ${entry.title}`);
      } else {
        logger.info(`Skipped existing dev log: ${entry.title}`);
      }
    }

    res.status(200).json({
      success: true,
      message: `Seeded ${created.length} new dev log entries (${devLogs.length - created.length} already existed)`,
      created: created.length,
      skipped: devLogs.length - created.length,
    });
  } catch (error) {
    logger.error('Error seeding dev logs', error instanceof Error ? error : new Error(String(error)), {
      endpoint: '/api/dev-log/seed',
      method: 'POST',
    });
    return res.status(500).json({
      error: 'Failed to seed dev logs',
