...
function getRank(count: number): { name: string; icon: string; color: string; next: number } {
  ...
  if (count >= 1) return { name: 'Pup', icon: '🐕', color: 'text-green-400', next: 5 };
  return { name: 'Lone Wolf', icon: '🌑', color: 'text-gray-400', next: 1 };
}
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {/* Referral Link */}
            <div className="mb-4">
              <label className="block text-xs text-gray-500 font-body mb-2 uppercase tracking-wide">
                Your Invite Link
              </label>
            ...
            {/* Rewards teaser */}
            <p className="text-center text-xs text-gray-600 mt-4 font-body">
